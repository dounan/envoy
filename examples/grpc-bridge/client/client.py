...
GRPC_METADATA = (('custom-cluster', 'dounan'),)
USAGE = """
...
class GrpcKVClient():
  ...
  def __init__(self):
    ...
    self.stub = kv.KVStub(channel)

  def get(self, key):
    r = kv.GetRequest(key=key)
    return self.stub.Get.with_call(r, metadata=GRPC_METADATA)

  def set(self, key, value):
    r = kv.SetRequest(key=key, value=value)
    return self.stub.Set.with_call(r, metadata=GRPC_METADATA)
